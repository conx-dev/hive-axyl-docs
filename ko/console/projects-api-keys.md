...
2. 멤버를 추가하려면 해당 콘솔 계정의 **Email**을 입력하고 **Role**(`OWNER` / `ADMIN` / `VIEWER`)을 선택한 뒤 **Add**를 클릭합니다.
3. 멤버 역할을 변경하려면 이름 옆 dropdown에서 새 역할을 선택합니다.
4. 멤버를 제거하려면 **Remove**를 클릭하고 dialog에서 확인합니다.
