...
2. **Name**, **Email**, **Password**(최소 8자), **Confirm password**를 입력합니다.
3. 양식을 제출하면 즉시 로그인되고 콘솔 홈으로 이동합니다.
...
| `ADMIN` | 프로젝트와 멤버를 관리할 수 있습니다(멤버 추가, 역할 변경, 멤버 제거). 단, `OWNER` 역할 멤버는 제외됩니다. |
...
**Dashboard** 메뉴는 선택한 프로젝트의 게임 지표를 보여줍니다. 일별 또는 월별 매출과 사용자 활동 요약을 market별로 나누어 보여주고, 날짜 범위, retention 기준, 국가, market으로 필터링할 수 있는 retention matrix를 제공합니다. 선택 기간의 국가 분포가 retention breakdown을 구성합니다. 게임 출시 후 일상적인 health check로 사용하세요.
