...
::: tip 배포 채널 준비 중
UPM registry 배포를 준비 중입니다. 그 전까지는 SDK를 local UPM package로 참조하세요.
:::

SDK package folder를 프로젝트에서 접근 가능한 위치로 복사하거나 clone한 뒤 `Packages/manifest.json`에 `file:` reference를 추가합니다.
...
경로는 프로젝트의 `Packages/` folder 기준 상대 경로입니다. Unity는 package와 bundled plugin을 자동으로 import합니다. Bundled sample app도 이 local-reference layout을 그대로 사용합니다.
