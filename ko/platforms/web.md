...
SDK는 첫 게스트 로그인에서 안정적인 설치 식별자를 만들고 저장합니다.
...
설치 식별자는 세션 저장소와 분리되며 `logout()` 후에도 유지됩니다. IdP 로그인은 이 값을 만들거나 사용하지 않습니다. 영구 브라우저 저장소를 사용할 수 없으면 네트워크 요청 전에 게스트 로그인이 실패합니다. 사이트 데이터를 지우면 새 게스트 계정이 생성될 수 있습니다. 이전 게스트 계정은 복구하지 못할 수 있습니다.
...
| `loginAsGuest()` | `Promise<Player>` | SDK가 관리하는 설치 식별자로 guest login |
