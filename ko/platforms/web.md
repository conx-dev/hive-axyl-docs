...
Hive Axyl Web SDK(`@hive-axyl/web-sdk`)는 브라우저 게임용 TypeScript SDK입니다. ConnectRPC를 통해 인증, 세션 영속성, 공지, 우편함, 결제를 처리합니다.

시작하기 전에 콘솔에서 프로젝트를 만들고 client API key를 발급하세요. [프로젝트와 API 키](/ko/console/projects-api-keys)를 참고하세요.

## 요구 사항

- 최신 브라우저(SDK는 ConnectRPC web transport를 통해 `fetch` 사용)
