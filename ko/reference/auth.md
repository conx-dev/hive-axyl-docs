...
인자가 없습니다. 첫 게스트 로그인에서 SDK가 암호학적으로 안전한 무작위 설치 식별자를 만들고 영구 저장소에 보관합니다. 이 값은 세션 저장소와 분리됩니다. `logout()`을 호출하거나 세션 저장을 비활성화해도 삭제되지 않습니다. IdP 로그인은 이 값을 만들거나 사용하지 않습니다.

로그인된 [`Player`](#player)를 반환합니다. SDK가 설치 식별자를 영구 저장할 수 없으면 네트워크 요청 전에 실패합니다. 앱 또는 사이트의 영구 데이터를 지우면 새 게스트 계정이 생성될 수 있습니다. 이전 게스트 계정은 복구하지 못할 수 있습니다.

새 게스트 생성은 `RATE_LIMITED`를 반환할 수 있습니다. 다시 시도하기 전에 에러 metadata의 `retry_after_seconds`를 사용하세요.
